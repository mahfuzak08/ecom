...
class VENDOR_Controller extends MX_Controller
{
    ...
    public function __construct()
    {
        ...
        $vars['vendor_mobile'] = $this->vendor_mobile;
    ...
    private function setVendorInfo()
    {
        ...
        if (isset($_SESSION['logged_vendor'])) {
            ...
            $this->vendor_mobile = $array['mobile'];
    ...
    private function saveNewVendorDetails()
    {
        ...
        if (!$this->Vendorprofile_model->isVendorUrlFree($_POST['vendor_url'], $this->vendor_id)) {
            $errors[] = lang('vendor_url_taken');
        }
        if (mb_strlen(trim($_POST['vendor_mobile'])) == 0) {
            $errors[] = lang('enter_vendor_mobile');
        }
        if (!$this->Vendorprofile_model->isVendorMobileFree($_POST['vendor_mobile'], $this->vendor_id)) {
            $errors[] = lang('vendor_mobile_taken');
        }
        if (empty($errors)) {
