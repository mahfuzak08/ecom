...
class Public_model extends CI_Model
{
    ...
    {
        $this->db->where('base_url', $host);
        return $this->db->get('shop_lists')->row_array();
    }
    ...
    {
        $select = 'products.id, products.quantity, products.image, products.url, products.size, products_translations.price, products_translations.title, products_translations.old_price';
        if($this->multiVendor == 1){
            $select .= ', vendors.url as vendor_url';
    ...
    public function getProducts($limit = null, $start = null, $big_get="", $vendor_id = false)
    {
        ...
        $select = 'products.id,products.image, products.quantity, products_translations.title, products_translations.price, products_translations.old_price, products.url, products.size';
        if($this->multiVendor == 1){
            $select .= ', vendors.url as vendor_url, vendors.name as vendor_name';
            $this->db->join('vendors', 'vendors.id = products.vendor_id', 'left');
        }
        $this->db->select($select);
        $this->db->join('products_translations', 'products_translations.for_id = products.id', 'left');
    ...
    private function getFilter($big_get)
    {
        ...
        if ($big_get['category'] != '') {
            ...
            $query = $this->db->query('SELECT id FROM shop_categories WHERE shop_id = '. SHOP_ID .' AND sub_for = ' . $this->db->escape($big_get['category']));
        ...
        if ($big_get['vendor_id'] != '') {
            $this->db->where('products.vendor_id = ', $big_get['vendor_id']);
        }
    ...
    public function getOneProduct($id)
    {
        ...
        $select = 'products.*, products_translations.title,products_translations.description, products_translations.price, products_translations.old_price, products.url, products.size, shop_categories_translations.name as categorie_name';
        if($this->multiVendor == 1){
            $select .= ', vendors.url as vendor_url, vendors.name as vendor_name';
            $this->db->join('vendors', 'vendors.id = products.vendor_id', 'left');
        }
    ...
    {
        $query = $this->db->query('SELECT SUM(IF(quantity<=0,1,0)) as out_of_stock, SUM(IF(quantity>0,1,0)) as in_stock FROM products WHERE visibility = 1 AND shop_id = '. SHOP_ID);
        return $query->row_array();
    }

    public function getShopItems($array_items)
    {
        $this->db->select('products.id, products.image, products.url, products.size, products.quantity, products_translations.price, products_translations.title');
    ...
    {
        $result = $this->db->query('SELECT email FROM users WHERE notify = 1 AND shop_id = '. SHOP_ID);
        $arr = array();
    ...
    /*
     * Users for notification by phone
     */

    public function getNotifyUsersPhone($id)
    {
        $result = $this->db->query("SELECT phone FROM customer WHERE id = $id AND shop_id =".SHOP_ID);
        $arr = array();
    ...
    public function setOrder($post)
    {
        ...
        $post['products'] = array();
        $sizes[] = array();
        $cartItems = $this->getCartItems(array_unique($post['id']));
        foreach ($post['id'] as $product) {
            $post['products'][$product] = $post['quantity'][$i];
            if(isset($post['size'])){
                $sizes[$product] = $post['size'][$i];
            }
            $i++;
        }
            ...
            foreach($post['products'] as $pr_id => $pr_qua) {
                foreach($cartItems as $item){
                    if($item["id"] == $pr_id){
                        ...
                        $item['quantity'] = $pr_qua;
                        if(count($sizes) > 0)
                            $item['size'] = $sizes[$pr_id];
                        $item['total'] = $pr_qua * $item['price'];
                        $products_to_order[] = [
                            'product_info' => $item,
    ...
    {
        $select = 'products.*, products_translations.price';
        if($this->multiVendor == 1){
            ...
            */
            foreach ($post['products'] as $product_id => $product_quantity) {
                ...
                if ($productInfo['vendor_id'] > 0) {
                    ...
                    //     $rr['order_id'] = 1233;
                    // }
                    $post['order_id'] = $post['last_order_id'];
    ...
    {
        $result = $this->db->insert('confirm_links', array('shop_id' => SHOP_ID, 'link' => $link, 'for_order' => $orderId));
        return $result;
    }

    public function getSliderProducts()
    {
        $select = 'products.id, products.quantity, products.image, products.url, products.size, products_translations.price, products_translations.title, products_translations.basic_description, products_translations.old_price';
        if($this->multiVendor == 1){
            $select .= ', vendors.name as vendor_name, vendors.url as vendor_url';
            $this->db->join('vendors', 'vendors.id = products.vendor_id', 'left');
        }
        $this->db->select($select);
        $this->db->join('products_translations', 'products_translations.for_id = products.id', 'left');
    ...
    {
        $result = $this->db->query("SELECT DATE_FORMAT(FROM_UNIXTIME(time), '%M %Y') as month, MAX(time) as maxtime, MIN(time) as mintime FROM blog_posts WHERE shop_id = '". SHOP_ID ."' GROUP BY DATE_FORMAT(FROM_UNIXTIME(time), '%M %Y')");
    ...
    public function registerUser($post)
    {
        ...
        // $this->sms_lib->sendSMS($post['phone'], "Your one time OTP is $otp. Please visite ".base_url()."login and then verify. \r\nThanks\r\n".$this->companyName."\r\nHelp: ".$this->footerContactPhone);
    ...
    public function checkPublicUserIsValid($post)
    {
        if(filter_var($post['email'], FILTER_VALIDATE_EMAIL)){
            ...
            $this->db->where('phone', $post['email']);
        }
    ...
    public function verify_phone($id)
    {
        $this->db->where('id', $id);
        $this->db->where('shop_id', SHOP_ID);
        $this->db->select('phone_verify');
        $result = $this->db->get('customer');
        $result = $result->row_array();
        if ($result['phone_verify'] == 'Yes') {
            return true;
        } else {
            return $result['phone_verify'];
        }
    }
    ...
    public function getProductsSuggestions($str)
    {
        $select = 'products.id,products.image, products_translations.title, products_translations.price, products.url, products.size';
        if($this->multiVendor == 1){
            $select .= ', vendors.url as vendor_url, vendors.name as vendor_name';
            $this->db->join('vendors', 'vendors.id = products.vendor_id', 'left');
        }
    ...
    public function getUserSuggestions($str)
    {
        $this->db->select('id, name, phone, email');
        $this->db->like('phone', $str);
        $this->db->where('shop_id', SHOP_ID);
    ...
    public function getUserProfileInfoByPhone($phone)
    {
        $this->db->where('phone', $phone);
        $this->db->where('shop_id', SHOP_ID);
        ...
        if($result == 1){
            ...
            $this->sms_lib->sendSMS($phone, "Your new OTP is $otp. Please use this OTP as your new password. \r\nThanks\r\n".$this->companyName."\r\nHelp: ".$this->footerContactPhone);
        }
        return $result;
    }


    public function getCartItems($array_items)
    {
        $this->db->select('products.*, pt.price, pt.title as name, pt.description');
        $this->db->from('products');
        $this->db->where('shop_id', SHOP_ID);
        if (count($array_items) > 1) {
            $i = 1;
        ...
        $this->db->join('products_translations as pt', 'pt.for_id = products.id', 'inner');
        $this->db->where('pt.abbr', MY_DEFAULT_LANGUAGE_ABBR);
        $query = $this->db->get();
        return $query->result_array();
    }

    public function setWish($post)
    {
        $i = 0;
        $this->db->trans_begin();
        foreach($post['id'] as $product_id) {
            if (!$this->db->insert('wish_list', array(
                        'shop_id' => SHOP_ID,
                        'date' => date("Y-m-d H:i:s"),
                        'customer_id' => $post['user_id'],
                        'product_id' => $product_id,
                        'quantity' =>$post['quantity'][$i],
                        'amount' =>$post['your_price'][$i]
                    ))) {
                log_message('error', print_r($this->db->error(), true));
            }
            $i++;
    ...
    public function update_counter($data){
        $query = $this->db->where('session_id', $data['session_id'])->get('visitor_counter');
        if ($query->num_rows() == 0) {
            if(! $this->db->insert('visitor_counter', array(
                'shop_id' => SHOP_ID,
    ...
    public function get_visitor_counter($today, $type)
    {
        if($today){
