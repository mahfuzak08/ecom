...
?>
<div class="container" id="view-product" data-id="<?= $product['id']; ?>">
    <div class="row">
        <div class="col-sm-4">
            <?php 
            ...
            <div <?= $product['folder'] != null ? 'style="margin-bottom:20px;"' : '' ?>>
                <img src="<?= $image; ?>" style="width:auto; height:auto;" data-num="0" class="other-img-preview img-responsive img-sl the-image" alt="<?= str_replace('"', "'", $product['title']) ?>">
            </div>
            <?php
        ...
        <div class="col-sm-8">
            ...
            <div class="row row-info">
                ...
                <div class="col-sm-6"><?= CURRENCY. price_format($product['price']) ?></div>
                <div class="col-sm-12 border-bottom"></div>
            </div>
            <?php if ($product['old_price'] != '') { ?>
                <div class="row row-info">
                    <div class="col-sm-6"><b><?= lang('old_price') ?>:</b></div>
                    <div class="col-sm-6"><?= CURRENCY. price_format($product['old_price']) ?></div>
                    <div class="col-sm-12 border-bottom"></div>
                </div>
            <?php } if ($publicQuantity == 1) { ?>
                <div class="row row-info">
                    ...
                    <div class="col-sm-6"><?= price_format($product['quantity'], 0) ?></div>
                    <div class="col-sm-12 border-bottom"></div>
                </div>
            <?php } ?>
            <div class="row row-info">
                    ...
                    if(!empty($cart)){
                        for($i=0; $i<count($cart); $i++)
                            if($cart[$i]->id == $product['id'])
                                echo price_format($cart[$i]->qty, 0);
                    }
                    else
                        echo 0;
                    ?></div>
            ...
            <div class="row row-info">
                <div class="col-xs-6"><b><?= lang('vendor') ?>:</b></div>
                <div class="col-xs-6"><b><?= $product['vendor_name'] ?></b></div>
                <div class="col-xs-12 border-bottom">

                </div>
            </div>
            <?php }?>
            <?php if($multiSize == 1 && $product['size'] != 'N' && $product['size'] != '') { ?>
            <div class="row row-info">
                <div class="col-xs-6"><b><?= lang('available_size') ?>:</b></div>
                <div class="col-xs-6">
                    <?php 
                    $sizes = explode(";", $product['size']);
                    for($i=0; $i<count($sizes); $i++){
                        $ss = explode("x", $sizes[$i]); ?>
                        <span class="size"><?= $ss[0]; ?></span>
                        <?php
                    }
                    ?>
            ...
            <div class="row row-info">
                ...
                <div class="col-sm-6">
                    <?php if ($product['quantity'] > 0) { ?>
                        <a href="javascript:void(0);" class="add2cart btn btn-success btn-sm btn-flat" data-id="<?= base64_encode($product['id']); ?>" data-title="<?= $product['title']; ?>" data-url="<?= $product['url']; ?>" data-size="<?= $product['size']; ?>" data-img="<?= $product['image']; ?>" data-price="<?= $product['price']; ?>">
                            <span class="text-to-bg"><?= lang('add_to_cart') ?></span>
                        </a>
                        <a href="javascript:void(0);" class="buy-now btn btn-blue btn-sm btn-flat" data-id="<?= base64_encode($product['id']); ?>" data-title="<?= $product['title']; ?>" data-url="<?= $product['url']; ?>" data-img="<?= $product['image']; ?>" data-price="<?= $product['price']; ?>" data-goto="<?= LANG_URL . '/checkout' ?>">
                            <span class="text-to-bg"><?= lang('buy_now') ?></span>
                        </a>
                        <?php if($wish_list == 1) { ?>
                            <a href="javascript:void(0);" class="wish-it btn btn-info btn-sm btn-flat" data-id="<?= base64_encode($product['id']); ?>" data-title="<?= $product['title']; ?>" data-url="<?= $product['url']; ?>" data-img="<?= $product['image']; ?>" data-price="<?= $product['price']; ?>" data-goto="<?= LANG_URL . '/wish' ?>">
