...
<form method="POST" action="" enctype="multipart/form-data">
    ...
    <div class="form-group for-shop">
        ...
        <input type="text" name="buy_price" placeholder="without currency at the end" value="<?= $trans_load != null && isset($trans_load[$language->abbr]['buy_price']) ? $trans_load[$language->abbr]['buy_price'] : '' ?>" class="form-control">
    </div>
    ...
    <div class="form-group for-shop">
        ...
        <select class="selectpicker form-control show-tick show-menu-arrow" data-live-search="true" name="shop_categorie">
            <?php foreach ($shop_categories as $shop_categorie) { 
                if($shop_categorie['sub_for'] == 0) { ?>
                    <option <?= isset($_POST['shop_categorie']) && $_POST['shop_categorie'] == $shop_categorie['id'] ? 'selected=""' : '' ?> value="<?= $shop_categorie['id'] ?>">
                        <?= $shop_categorie['name'] ?>
                    </option>
                <?php } ?>
                <?php foreach ($shop_categories as $sub_categorie) { 
                    if($shop_categorie['id'] == $sub_categorie['sub_for']) { ?>
                        <option <?= isset($_POST['shop_categorie']) && $_POST['shop_categorie'] == $sub_categorie['id'] ? 'selected=""' : '' ?> value="<?= $sub_categorie['id'] ?>">
                            <?= "&nbsp;&nbsp;&nbsp;&nbsp;#".$sub_categorie['name'] ?>
                        </option>
                    <?php } ?>
                <?php } ?>
                ...
                <?php if(isset($_POST['size']) && $_POST['size'] != 'N' && $_POST['size'] != '') {
                    ...
                    for($n=0; $n<count($sizes); $n++){ 
                        ...
                        <div class="size_row">
                            ...
                            <input type="text" placeholder="Quantity" name="qty[]" value="<?= $sq[1]; ?>" class="form-control col_cus_100px">&nbsp;
                            <?php if($n==0) { ?> 
    ...
    <div class="form-group for-shop">
        ...
        <input type="hidden" name="old_quantity" value="<?= @$_POST['quantity'] ?>">
    </div>
    <div class="form-group for-shop">
        <label>Reorder Level</label>
        <input type="text" placeholder="number" name="reorder_level" value="<?= @$_POST['reorder_level'] ?>" class="form-control" id="reorder_level">
    </div>
        ...
        <div class="form-group for-shop">
            ...
            <select class="selectpicker" data-live-search="true" name="vendor_id">
                <?php foreach ($vendor as $row) { ?>
                    <option <?= isset($_POST['vendor_id']) && $_POST['vendor_id'] == $row['id'] ? 'selected' : '' ?> value="<?= $row['id'] ?>"><?= $row['name'] ?></option>
                <?php } ?>
            </select>
        </div>
    <?php } if ($showBrands == 1) { ?>
        <div class="form-group for-shop">
            ...
            <select class="selectpicker" data-live-search="true" name="brand_id">
                <?php foreach ($brands as $brand) { ?>
                    <option <?= isset($_POST['brand_id']) && $_POST['brand_id'] == $brand['brand_id'] ? 'selected' : '' ?> value="<?= $brand['brand_id'] ?>"><?= $brand['name'] ?></option>
                <?php } ?>
            </select>
        </div>
	...
	<div class="form-group for-shop">
        ...
        <input type="text" name="barcode" placeholder="Enter the barcode here" value="<?= isset($_POST['barcode']) && $_POST['barcode'] != null && $_POST['barcode'] != '' ? $_POST['barcode'] : '' ?>" class="form-control">
    </div>
	<?php } ?>
