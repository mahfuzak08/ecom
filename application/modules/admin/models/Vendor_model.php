...
class Vendor_model extends CI_Model
{
    ...
    public function getvendors()
    {
        $this->db->where("is_delete<2");
        $this->db->where('shop_id', SHOP_ID);
        $query = $this->db->select('id as vendor_id, name as vendor_name, url as vendor_address, mobile as vendor_mobile, email as vendor_email, balance, is_delete as active')->get('vendors');
        return $query->result();
    }
    ...
    public function setVendor($post, $id = 0)
    {
        ...
        if ($id > 0) {
            ...
            if (!$this->db->where('id', $id)->update('vendors', array(
						...
						'balance' => isset($post['balance']) && $post['balance'] != 0 ? $post['balance'] : 0
                    ))) {
                log_message('error', print_r($this->db->error(), true));
            }
        } else {
	...
	public function checkVendorExsists($post)
    {
        ...
        $this->db->where('mobile', $post['vendor_mobile']);
        $this->db->where('shop_id', SHOP_ID);
        $query = $this->db->get('vendors');
        $row = $query->row_array();
