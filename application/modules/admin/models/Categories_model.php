...
class Categories_model extends CI_Model
{
    ...
    public function getShopCategories2($limit = null, $start = null)
    {
        $limit_sql = '';
        if ($limit !== null && $start !== null) {
            $limit_sql = ' LIMIT ' . $start . ',' . $limit;
        ...
        $query = $this->db->query('SELECT sc.*, sct.name FROM `shop_categories` as sc JOIN `shop_categories_translations` as sct ON sc.id = sct.for_id WHERE sc.shop_id = '. SHOP_ID .' AND sct.abbr = "en" ORDER BY position ASC ' . $limit_sql);
    ...
    {
        $this->db->trans_begin();
        if (!$this->db->insert('shop_categories', array('shop_id' => SHOP_ID, 'sub_for' => $post['sub_for']))) {
            log_message('error', print_r($this->db->error(), true));
        }
